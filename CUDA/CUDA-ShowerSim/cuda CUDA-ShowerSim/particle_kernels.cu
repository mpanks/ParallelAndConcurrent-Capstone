    ...
    int particleCount)
{
    ...
    if (p.position.y <= 0.0f) {
        Kernel_RespawnParticle(p, &states[i]);
    }
...
{
    const int threadsPerBlock = 256;
    const int blocks = (particle_count + threadsPerBlock - 1) / threadsPerBlock;

    UpdateParticles<<<blocks, threadsPerBlock>>>(d_particles, d_states, dt, gravity, particle_count);
